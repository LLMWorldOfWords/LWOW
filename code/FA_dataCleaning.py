...
data_path = os.path.join(base_dir, 'data/mapping_tables/EnglishCustomDict.txt')
...
FA_data['Mistral'] = FA_df(loadData(os.path.join(base_dir, 'data/original_datasets/Mistral/mistral_free_associations.jsonl')))
FA_data['Llama3'] = FA_df(loadData(os.path.join(base_dir, 'data/original_datasets/Llama3/llama3-8b_free_associations.jsonl')))
FA_data['Haiku'] = FA_df(loadData(os.path.join(base_dir, 'data/original_datasets/Haiku/haiku_free_associations.jsonl')))
FA_data['GPT-3.5 Turbo'] = FA_df(loadData(os.path.join(base_dir, 'data/original_datasets/GPT/gpt-3.5_turbo125_male-female.jsonl')))
FA_data['GPT-4o'] = FA_df(loadData(os.path.join(base_dir, 'data/original_datasets/GPT/gpt-4o-2024-05-13_male-female.jsonl')))
FA_data['GPT-4 Turbo'] = FA_df(loadData(os.path.join(base_dir, 'data/original_datasets/GPT/gpt-4-turbo-2024-04-09_male-female.jsonl')))
...
for model, data in FA_clean_dfs.items():
    data.to_csv(os.path.join(base_dir, 'data/processed_datasets/FA_datasets/FA_' + model + '.csv', index = False))
...
df.to_csv(os.path.join(base_dir, 'data/summary_tables/FA_summary_stats.csv', index = False))
